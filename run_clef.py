...
def one_experiment(opts):
    ...
    opts.out_fold = './result-clef/' + opts.data + '/' + opts.query +'/' + opts.self_define_name  # decided by dataset/query
    ...
    args_pool = {'MNIST':
                    {'n_epoch': 100, 'transform': transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))]),
                ...
                'domainnet':
                        {'n_epoch': 1, 'transform': pre_process.image_train(),
                        'loader_tr_args':{'batch_size': 32, 'num_workers': 1},
                        'loader_label_args':{'batch_size': 32, 'num_workers': 1},
...
# python run3clef.py > log/clef_rd_one_hot_wobn.log&
