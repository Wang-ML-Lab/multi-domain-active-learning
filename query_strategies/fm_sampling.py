...
class FMSampling(Strategy):
	def __init__(self, train_set, test_set, handler, net, opts):
		super(FMSampling, self).__init__(train_set, test_set, handler, net, opts)
	...
	def query(self, alpha, rd):
		...
		for i in range(self.dm_num):
			...
			if per_domain_num[i]==0:
				...
				eng_num = int(per_domain_num[i]*self.eng_w)
				mg_num = per_domain_num[i] - eng_num
				#free energy——————————————————————————————————————————————————————————————————————————————
