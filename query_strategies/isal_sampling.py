...
class ISAL_Sampling(Strategy):
	def __init__(self, train_set, test_set, handler, net, opts):
		super(ISAL_Sampling, self).__init__(train_set, test_set, handler, net, opts)
	...
	def query(self, alpha, rd):
		...
		for i in range(self.dm_num):
			...
			if per_domain_num[i]==0:
				...
				U = self.predict_influ(self.X[idxs_unlabeled], self.Y[idxs_unlabeled], self.dm[idxs_unlabeled], self.idxs_lb[idxs_unlabeled])
				
				new_add = idxs_unlabeled[U.sort()[1]][:per_domain_num[i]] # ascending
			#——————————————————————————————————————————————————————————————————————————————
